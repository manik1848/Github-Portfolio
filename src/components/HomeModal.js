...
function setModal(value, valObj, setLgShow) {
	if (value === 'Projects') {
		...
		return <SocialModal SocialModalObj={valObj} setLgShow={setLgShow}/>
...
function HomeModal({lgShow, setLgShow, HomeCards}) {
	return (
		...
		>
			...
			<Modal.Body>{setModal(HomeCards.title, HomeCards.value, setLgShow)}</Modal.Body>
		</Modal>
	)
}
